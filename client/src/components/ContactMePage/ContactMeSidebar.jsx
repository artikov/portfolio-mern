...
const ContactMeSidebar = () => {
	...
	return (
		<div>
			<div>
				<div
					className={`text-white text-sm flex gap-2 p-2.5 border-b border-slate-800`}
				...
				<div
					className={`m-4 text-slate-500 text-sm flex flex-col gap-4 accordion-content ${
						isContactsOpen ? "open" : ""
					}`}
				>
					{isLoading ? (
			...
			<div>
				<div
					className="text-white text-sm flex gap-2 p-2.5 border-b border-t border-slate-800"
				...
				<div
					className={`m-4 text-slate-500 flex flex-col gap-2 text-sm accordion-content ${
						isLinksOpen ? "open" : ""
					}`}
				>
					{links?.map((link) => (
